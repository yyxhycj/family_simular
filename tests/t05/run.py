"""Run the focused T05 optional-editor contract against production Lua modules."""
import argparse
import hashlib
import json
import subprocess
...
def main():
    ...
    try:
        ...
        case = lua.execute((ROOT / "tests/t05/contract.lua").read_text())
        result = {"id": "t05_contract", "status": "PASS", "evidence": plain(case.run())}
        lua, _ = runtime(storage)
        case = lua.execute((ROOT / "tests/t05/contract.lua").read_text())
        result["restart"] = plain(case.restart())
    except Exception as error:
        result = {"id": "t05_contract", "status": "FAIL", "error": repr(error)}
    print(result["id"], result["status"])
    paths = [
        *sorted((ROOT / "scripts/Jiaye").glob("*.lua")),
        ROOT / "tests/t01/boundary.lua", ROOT / "tests/t01/run.py",
        ROOT / "tests/t05/contract.lua", ROOT / "tests/t05/run.py",
    ]
    manifest = {
