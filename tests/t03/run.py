"""Run the focused T03 generator contract against production Lua modules."""
import argparse
import hashlib
import json
import subprocess
...
def main():
    ...
    try:
        ...
        case = lua.execute((ROOT / "tests/t03/contract.lua").read_text())
        run = case.run if lua_type(case) == "table" else case
        result = {"id": "t03_contract", "status": "PASS", "evidence": plain(run())}
        if lua_type(case) == "table" and case.restart is not None:
            lua, _ = runtime(storage)
            case = lua.execute((ROOT / "tests/t03/contract.lua").read_text())
            result["restart"] = plain(case.restart())
    except Exception as error:
        result = {"id": "t03_contract", "status": "FAIL", "error": repr(error)}
    print(result["id"], result["status"])
    paths = [
        *sorted((ROOT / "scripts/Jiaye").glob("*.lua")),
        ROOT / "tests/t01/boundary.lua", ROOT / "tests/t01/run.py",
        ROOT / "tests/t03/contract.lua", ROOT / "tests/t03/run.py",
    ]
    manifest = {
